    ...
    for batch_idx in range(num_batches):
        ...
        for i, (img, confT, confS, ssimT, ssimS, nccT, nccS, idx) in enumerate(zip(images[start_idx:end_idx], generation_confidences_for_target[start_idx:end_idx], generation_confidences_for_similarity[start_idx:end_idx], generation_ssims_for_target[start_idx:end_idx], generation_ssims_for_similarity[start_idx:end_idx], generation_nccs_for_target[start_idx:end_idx], generation_nccs_for_similarity[start_idx:end_idx], indices[start_idx:end_idx])):
            ...
            if experiment_number == "2_1b":
                ...
                axs[row, col].set_title(f'Gen {idx}\nConf-{target_digit_for_confidence}: {confS:.4f}\nSSIM-{target_digit_for_confidence}: {ssimS:.4f}\nNCC-{target_digit_for_confidence}: {nccS:.4f}', fontsize=8)
...
def plot_scores_vs_generations(generations, generation_confidences_for_target, generation_confidences_for_similarity, generation_ssims_for_target, generation_ssims_for_similarity, generation_nccs_for_target, generation_nccs_for_similarity, output_subdir, target_digit_for_confidence, target_digit_for_similarity, replicate, model_name, dataset_name, experiment_number):
    ...
    plt.ylabel("Score")
    plt.title(f"Scores vs. Gen. (Experiment: {experiment_number}, Model/Dataset: {model_name}/{dataset_name}, Target Class: {target_digit_for_confidence}, Replicate: {replicate})")
    plt.legend()
    plt.grid(True)
